...
impl X86_64Codegen {
    ...
    fn emit_modrm(&mut self, modrm_low: u8, rm: Operand) {
        match rm {
            Operand::Reg(_) => {
                self.emit8(((ModRM::Reg as u8) << 6) | modrm_low);
            }
            Operand::Mem(_, offset) => {
                self.emit8(((ModRM::MemDisp32 as u8) << 6) | modrm_low);
    ...
    pub fn mov(&mut self, dst: Operand, src: Operand) {
        match (dst, src) {
            ...
            (Operand::Reg(dst_reg), Operand::Imm64(imm)) => {
                self.emit_rex_oi(dst, 1);
                self.emit8(0xb8 | Self::encode_reg(dst_reg));
                self.emit64(imm as u64);
            }
            // mov [base_reg + offset], reg
            (Operand::Mem(_, _), Operand::Reg(src_reg)) => {
                self.emit_rex_mr(dst, src, 1);
                self.emit8(0x89);
                self.emit_modrm_mr(dst, src_reg);
            }
            // mov [base_reg + offset], imm
            (Operand::Mem(_, _), Operand::Imm64(imm)) => {
                self.emit_rex_slash(dst, 1);
                self.emit8(0xc7);
                self.emit_modrm_slash(0, dst);
                self.emit32(imm as u32);
            ...
            (Operand::Reg(dst_reg), Operand::Mem(_, _)) => {
    ...
    pub fn push(&mut self, src: Operand) {
        match src {
            Operand::Reg(src_reg) => {
                self.emit_rex_oi(src, 1);
                self.emit8(0x50 | Self::encode_reg(src_reg));
            }
            Operand::Imm64(imm) => {
                self.emit8(0x68);
                self.emit32(imm as u32);
    ...
    pub fn pop(&mut self, dst: Operand) {
        match dst {
            Operand::Reg(dst_reg) => {
                self.emit_rex_oi(dst, 1);
                self.emit8(0x58 | Self::encode_reg(dst_reg));
            }
            _ => unimplemented!(),
        }
    }
...
mod tests {
    ...
    fn test_x86_64_codegen_mov() {
        ...
        codegen.mov(Mem(Rax, 0x1337), Reg(Rbx));
        codegen.mov(Reg(Rax), Mem(Rbx, 0x41414141));
        codegen.mov(Reg(R8), Imm64(0x1234));
        codegen.mov(Reg(Rsp), Reg(R15));
        codegen.mov(Mem(R15, 0x12345678), Imm64(0x41424344));
